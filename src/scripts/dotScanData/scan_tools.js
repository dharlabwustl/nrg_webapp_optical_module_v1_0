...
function ScanEditor(_sessionID,_scanID,_options){
	...
	this.render=function(){
        if (this.scan) {
            ...
            td1.innerHTML = "Scan Field 1:";
            td1.align = "left";
            if (this.scan.extension.scan_field_1 != undefined && this.scan.extension.scan_field_1 != null)
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/scan_field_1'>" + this.scan.extension.scan_field_1 + "</textarea>";
            else
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/scan_field_1'></textarea>";
            ...
            td1.innerHTML = "Scan Field 2:";
            td1.align = "left";
            if (this.scan.extension.scan_field_2 != undefined && this.scan.extension.scan_field_2 != null)
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/scan_field_2'>" + this.scan.extension.scan_field_2 + "</textarea>";
            else
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/scan_field_2'></textarea>";
...
function renderScans(scans,tbody_id,session_id,project){
	...
	for(var scanC=0;scanC<scans.length;scanC++){
		...
		//scan_field_1
                td= document.createElement("td");
                if(scan.scan_field_1){
		    td.innerHTML=scan.scan_field_1;
                }
                tr.appendChild(td);
		
		//scan_field_2
                td= document.createElement("td");
                if(scan.scan_field_2){
		    td.innerHTML=scan.scan_field_2;
                }
                tr.appendChild(td);
...
function ScanSet(_options,_scans){
	...
	this.completeScanLoad=function (obj1){
		...
		for(var slC=0;slC<tempScans.length;slC++){
		    ...
		    tempScan.setProperty("scan_field_1",tempScans[slC].scan_field_1);
		    tempScan.setProperty("scan_field_2",tempScans[slC].scan_field_2);
		...
		for(var csC=0;csC<this.new_scans.length;csC++){
			...
			if(scan.id_input.value==""){
		  		if(scan.type_input.value!="" || 
				   ...
				   || !((scan.scan_field_1_input.value=="")||(scan.scan_field_1_input.value=="NULL"))
				   || !((scan.scan_field_2_input.value=="")||(scan.scan_field_2_input.value=="NULL"))){
		  			removeAppendImage(scan.type_input);
...
function scanListingEditor(_tbody,_scanSet,_options){
	...
	this.render=function(){
		...
		for(var scanC=0;scanC<temp_scans.length;scanC++){
			...
			//scan field 1
                        td= document.createElement("td");
                        if(scan.scan_field_1_input==undefined){
			    scan.scan_field_1_input=document.createElement("input");
			    scan.scan_field_1_input.type="text";
			    if(scan.ScanField1!=undefined && scan.ScanField1 != "NULL"){
				scan.scan_field_1_input.value=scan.ScanField1;
			    }
			    scan.scan_field_1_input.size="40";
                        }
			scan.scan_field_1_input.id=elementName + scanXPath(modality,scanTypeTable) + "/scan_field_1";
			scan.scan_field_1_input.name=elementName + scanXPath(modality,scanTypeTable) + "/scan_field_1";
                        scan.scan_field_1_input.className += " nullable";
                        td.appendChild(scan.scan_field_1_input);
                        tr.appendChild(td);

			//scan field 2
                        td= document.createElement("td");
                        if(scan.scan_field_2_input==undefined){
                            scan.scan_field_2_input=document.createElement("input");
                            scan.scan_field_2_input.type="text";
                            if(scan.ScanField2!=undefined && scan.ScanField2 != "NULL"){
                                scan.scan_field_2_input.value=scan.ScanField2;
                            }
                            scan.scan_field_2_input.size="40";
                        }
                        scan.scan_field_2_input.id=elementName + scanXPath(modality,scanTypeTable) + "/scan_field_2";
                        scan.scan_field_2_input.name=elementName + scanXPath(modality,scanTypeTable) + "/scan_field_2";
                        scan.scan_field_2_input.className += " nullable";
                        td.appendChild(scan.scan_field_2_input);
                        tr.appendChild(td);
