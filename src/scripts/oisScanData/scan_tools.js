/*
 * D:/Development/XNAT/1.6/xnat_builder_1_6dev/plugin-resources/webapp/xnat/scripts/dotScanData/scan_tools.js
...
function getNominalType(scan) {
    if (!scan.extension && scan.constructor.name === "opti_dotScanData") {
...
function getImageScanNominalType(scan) {
    if (scan.Type) {
        return scan.Type;
    } else if (scan.SeriesDescription) {
        return scan.SeriesDescription;
    } else {
        return "Unknown";
    }
}
...
function ScanEditor(_sessionID,_scanID,_options){
	...
	this.render=function(){
        if (this.scan) {
            ...
            td1.innerHTML = "Pixels X:";
            td1.align = "left";
            if (this.scan.extension.pixels_x != undefined && this.scan.extension.pixels_x != null)
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/pixels_x'>" + this.scan.extension.pixels_x + "</textarea>";
            else
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/pixels_x'></textarea>";
            ...
            td1.innerHTML = "Pixels Y:";
            td1.align = "left";
            if (this.scan.extension.pixels_y != undefined && this.scan.extension.pixels_y != null)
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/pixels_y'>" + this.scan.extension.pixels_y + "</textarea>";
            else
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/pixels_y'></textarea>";
	    ...
	    //frames led
            ...
            td1.innerHTML = "Frames Led:";
            td1.align = "left";
            if (this.scan.extension.frames_led != undefined && this.scan.extension.frames_led != null)
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/frames_led'>" + this.scan.extension.frames_led + "</textarea>";
            else
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/frames_led'></textarea>";
	    ...
	    //frames hb
            ...
            td1.innerHTML = "Frames Hb:";
            td1.align = "left";
            if (this.scan.extension.frames_hb != undefined && this.scan.extension.frames_hb != null)
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/frames_hb'>" + this.scan.extension.frames_hb + "</textarea>";
            else
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/frames_hb'></textarea>";
	    ...
	    //contrasts hb
            ...
            td1.innerHTML = "Contrasts Hb:";
            td1.align = "left";
            if (this.scan.extension.contrasts_hb != undefined && this.scan.extension.contrasts_hb != null)
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/contrasts_hb'>" + this.scan.extension.contrasts_hb + "</textarea>";
            else
                td2.innerHTML = "<textarea class='nullable' cols='30' rows='4' name='" + modality + "/contrasts_hb'></textarea>";
	...
	for(var scanC=0;scanC<scans.length;scanC++){
		...
		//pixels_y
		    ...
		    td.innerHTML=scan.pixels_y;
                ...
                td= document.createElement("td");
                if(scan.frames_hb){
                    td.innerHTML=scan.frames_hb;
                }
                tr.appendChild(td);

                //contrasts_hb
                td= document.createElement("td");
                if(scan.contrasts_hb){
                    td.innerHTML=scan.contrasts_hb;
                }
                tr.appendChild(td);
...
function ScanSet(_options,_scans){
	...
	this.completeScanLoad=function (obj1){
		...
		for(var slC=0;slC<tempScans.length;slC++){
		    ...
		    tempScan.setProperty("pixels_x",tempScans[slC].pixels_x);
		    tempScan.setProperty("pixels_y",tempScans[slC].pixels_y);
                    tempScan.setProperty("frames_led",tempScans[slC].frames_led);
                    tempScan.setProperty("frames_hb",tempScans[slC].frames_hb);
                    tempScan.setProperty("contrasts_hb",tempScans[slC].contrasts_hb);
			...
			if(scan.id_input.value==""){
		  		if(scan.type_input.value!="" || 
				   ...
				   || !((scan.note_input.value=="")||(scan.note_input.value=="NULL"))
                                   || !((scan.pixels_x_input.value=="")||(scan.pixels_x_input.value=="NULL"))
                                   || !((scan.pixels_y_input.value=="")||(scan.pixels_y_input.value=="NULL"))
                                   || !((scan.frames_led_input.value=="")||(scan.frames_led.value=="NULL"))
				   || !((scan.frames_hb.value=="")||(scan.frames_hb.value=="NULL"))
				   || !((scan.contrasts_hb_input.value=="")||(scan.contrasts_hb_input.value=="NULL"))){
		  			removeAppendImage(scan.type_input);
...
function scanListingEditor(_tbody,_scanSet,_options){
	...
	this.render=function(){
		...
		for(var scanC=0;scanC<temp_scans.length;scanC++){
			...
			//pixels_x
                        td= document.createElement("td");
                        if(scan.pixels_x_input==undefined){
			    scan.pixels_x_input=document.createElement("input");
			    scan.pixels_x_input.type="text";
			    if(scan.PixelsX!=undefined && scan.PixelsX != "NULL"){
				scan.pixels_x_input.value=scan.PixelsX;
			    }
			    scan.pixels_x_input.size="40";
                        }
			scan.pixels_x_input.id=elementName + scanXPath(modality,scanTypeTable) + "/pixels_x";
			scan.pixels_x_input.name=elementName + scanXPath(modality,scanTypeTable) + "/pixels_x";
                        scan.pixels_x_input.className += " nullable";
                        td.appendChild(scan.pixels_x_input);
                        tr.appendChild(td);

			//pixels_y
                        td= document.createElement("td");
                        if(scan.pixels_y_input==undefined){
                            scan.pixels_y_input=document.createElement("input");
                            scan.pixels_y_input.type="text";
                            if(scan.PixelsY!=undefined && scan.PixelsY != "NULL"){
                                scan.pixels_y_input.value=scan.PixelsY;
                            }
                            scan.pixels_y_input.size="40";
                        }
                        scan.pixels_y_input.id=elementName + scanXPath(modality,scanTypeTable) + "/pixels_y";
                        scan.pixels_y_input.name=elementName + scanXPath(modality,scanTypeTable) + "/pixels_y";
                        scan.pixels_y_input.className += " nullable";
                        td.appendChild(scan.pixels_y_input);
                        tr.appendChild(td);

                        //frames_led
                        td= document.createElement("td");
                        if(scan.frames_led_input==undefined){
                            scan.frames_led_input=document.createElement("input");
                            scan.frames_led_input.type="text";
                            if(scan.FramesLed!=undefined && scan.FramesLed != "NULL"){
                                scan.frames_led_input.value=scan.FramesLed;
                            }
                            scan.frames_led_input.size="40";
                        }
                        scan.frames_led_input.id=elementName + scanXPath(modality,scanTypeTable) + "/frames_led";
                        scan.frames_led_input.name=elementName + scanXPath(modality,scanTypeTable) + "/frames_led";
                        scan.frames_led_input.className += " nullable";
                        td.appendChild(scan.frames_led_input);
                        tr.appendChild(td);

                        //frames_hb
                        td= document.createElement("td");
                        if(scan.frames_hb_input==undefined){
                            scan.frames_hb_input=document.createElement("input");
                            scan.frames_hb_input.type="text";
                            if(scan.FramesHb!=undefined && scan.FramesHb != "NULL"){
                                scan.frames_hb_input.value=scan.FramesHb;
                            }
                            scan.frames_hb_input.size="40";
                        }
                        scan.frames_hb_input.id=elementName + scanXPath(modality,scanTypeTable) + "/frames_hb";
                        scan.frames_hb_input.name=elementName + scanXPath(modality,scanTypeTable) + "/frames_hb";
                        scan.frames_hb_input.className += " nullable";
                        td.appendChild(scan.frames_hb_input);
                        tr.appendChild(td);

                        //contrasts_hb
                        td= document.createElement("td");
                        if(scan.contrasts_hb_input==undefined){
                            scan.contrasts_hb_input=document.createElement("input");
                            scan.contrasts_hb_input.type="text";
                            if(scan.ContrastsHb!=undefined && scan.ContrastsHb != "NULL"){
                                scan.contrasts_hb_input.value=scan.ContrastsHb;
                            }
                            scan.contrasts_hb_input.size="40";
                        }
                        scan.contrasts_hb_input.id=elementName + scanXPath(modality,scanTypeTable) + "/contrasts_hb";
                        scan.contrasts_hb_input.name=elementName + scanXPath(modality,scanTypeTable) + "/contrasts_hb";
                        scan.contrasts_hb_input.className += " nullable";
                        td.appendChild(scan.contrasts_hb_input);
                        tr.appendChild(td);
